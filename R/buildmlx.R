                      ...
                      FDR_thr=0.1)
{
  ...
  if (identical(covToTest, "all"))
    covFix = NULL
  else covFix <- setdiff(Rsmlx:::mlx.getCovariateInformation()$name,
                         covToTest)
  if (iop.ll) {
    ll.prev <- Inf
    ll.new <- ll.ini
  ...
  while (!stop.test) {
    ...
    if (model$correlation & corr.test) {
      ...
      res.correlation <- lapply(Rsmlx:::mlx.getIndividualParameterModel()$correlationBlocks$id,
                                sort)
    }
  ...
  if (test) {
    ...
    Rsmlx:::print_result(print, summary.file, to.cat = to.cat, to.print = NULL)
    if (model$covariate & sum(Rsmlx:::mlx.getIndividualParameterModel()$variability$id) >
        1) {
      ...
      while (test.cor) {
        ...
        if (length(i.min) > 0) {
          ...
          if (test.cor) {
            ...
            Rsmlx:::print_result(print, summary.file, to.cat = NULL,
                                 to.print = to.print)
  ...
  if (model$correlation) {
    correlation.model.print <- lapply(Rsmlx:::mlx.getIndividualParameterModel()$correlationBlocks$id,
                                      sort)
